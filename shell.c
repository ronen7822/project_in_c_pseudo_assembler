...
// Student name: Yair Korngut, ID: 318635521
#include <ctype.h>
#include <errno.h>
#include <fcntl.h>
#include <glob.h>
...
int runCommand(struct job newJob, struct jobSet * jobList, 
    ...
    if (!strcmp(newJob.progs[0].argv[0], "exit")) {
		...
		if (newJob.progs[0].argv[1] == NULL) // need to correct
            newdir = getenv("HOME");
        else 
		...
		statusString = calloc(8, sizeof(char)); // "Running" and "Stopped" length is 7
		
		// scan the job list and print the status
		for (job = jobList->head; job; job = job->next) {	
			if (job->runningProgs - job->stoppedProgs > 0) strcpy(statusString, "Running");
			else strcpy(statusString, "Stopped");
    	...
    	return 0;
	 	...
	 	//wrong number of arguments
			printf("%s: exactly one argument is expected\n", newJob.progs[0].argv[0]);
			return(1);
		} else if (!((newJob.progs[0].argv[1])[0] == '%') || //argument is not %N
					!atoi(newJob.progs[0].argv[1] + sizeof(char))) {
			printf("%s: bad argument %s\n", newJob.progs[0].argv[0], newJob.progs[0].argv[1]);
			return(1);
		} 
	
		// jobNum is the jobId to perform the action
		jobNum = atoi(newJob.progs[0].argv[1] + sizeof(char));
	
		for (job = jobList->head; job; job = job->next) {
			if (job->jobId == jobNum) break;
		}
		if (!job) {
			printf("%s: unkown job %s\n", newJob.progs[0].argv[0], newJob.progs[0].argv[1] + sizeof(char));
			return(1);
		}

		// if the job should move to foreground
		if ((newJob.progs[0].argv[0])[0] == 'f') {
			if(tcsetpgrp(0, job->pgrp)) {
				perror("tcsetpgrp");
				return(1);
			}
			jobList->fg = job; //set jobList fg to be the selected job
		}
		
		// if the job is "stopped" now
		if (job->runningProgs - job->stoppedProgs == 0) {
			if(kill(-job->pgrp, SIGCONT)) {
				printf("could not continue the job\n");
				return(1);
			}
			// set all childProgs to be running
			job->stoppedProgs = 0;
			for (progNum = 0; progNum < job->numProgs; progNum++)
				job->progs[progNum].isStopped = 0;
		}
